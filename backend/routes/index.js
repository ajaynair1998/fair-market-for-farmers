...
router.get("/profile", async (req, res) => {
  try {
    ...
    // if not authorised
    if (!userName)
      res.status(404).json({ success: false, msg: "you are unauthorised" });
    else {
      // if authorised
      let userDetails = await mongoDbOperations.getProfileDetailsOfTheUser(
        userName
      );
...
router.post("/profile", async (req, res) => {
  try {
    ...
    if (!userName)
      ...
      // if authorised
      let successfullyEdited =
        await mongoDbOperations.editProfileDetailsOfTheUser(userName, req.body);
  ...
  try {
    ...
    if (!userName)
      ...
      let productsToDisplay =
        await mongoDbOperations.retrieveAllProductExceptUsersProduct(userName);
      res
        .status(200)
        .json({
          success: true,
          ...
          user: userName,
          dashBoardProducts: productsToDisplay,
        });
    }
  } catch (err) {
  ...
  try {
    ...
    if (!userName) {
      ...
      if (!req.body.productId) {
        ...
        let productDetails = await mongoDbOperations.retrieveProductDetails(
          req.body.productId
        );
...
router.post("/product", async (req, res) => {
  try {
    ...
    if (!userName || !req.body.product)
      ...
      let productAddedSuccessfully = await mongoDbOperations.addProductByUser(
        userName,
        req.body.product
      );
      if (productAddedSuccessfully)
        ...
        res
          .status(500)
          .json({ success: false, msg: "could not add the product" });
    }
  } catch (err) {}
});
...
router.post("/buy", async (req, res) => {
  try {
    ...
    if (!userName) {
      ...
      if (!req.body.product || !req.body.stock)
      ...
      let reduceAvailableStock =
        await mongoDbOperations.reduceStockBeforeTransaction(
          productId,
          req.body.stock
        );
      ...
      let transaction = await mongoDbOperations.makeTransactionByBuyer(
        userName,
        req.body.stock,
        productObject
      );
      ...
      if (transaction)
        res
          .status(200)
          .json({ success: true, msg: "Transaction Done Successfully" });
...
router.get("/myproducts", async (req, res) => {
  try {
    ...
    if (!userName)
      ...
      let productsTheUserIsSelling =
        await mongoDbOperations.retrieveAlltheProductByUser(userName);
      res
        .status(200)
        .json({ success: true, products: productsTheUserIsSelling });
    }
  } catch (err) {
    console.log(err);
  }
...
router.get("/transactions", async (req, res) => {
  try {
    ...
    if (!userName)
      ...
      let transactionsOfTheUser =
        await mongoDbOperations.retriveUsersTransactions(userName);
      res
        .status(200)
        .json({ success: true, transactions: transactionsOfTheUser });
    }
  } catch (err) {
    console.log(err);
  }
